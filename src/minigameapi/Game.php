...
abstract class Game {
	...
	final public function update(int $updateCycle) {
		...
		if($this->isWaiting()) {
			...
			if ($this->getMiniGameApi()->getConfig()->get('show-left-time') and $this->getRemainingWaitTime()->asSec() <= $this->getMiniGameApi()->getConfig()->get('show-left-time'))$this->broadcastMessage($this->getMiniGameApi()->getLanguage()->translateString('left.time',[intval($this->getRemainingWaitTime()->asSec())]));
            ...
            if ($this->getMiniGameApi()->getConfig()->get('show-left-time') and $this->getRemainingRunTime()->asSec() <= $this->getMiniGameApi()->getConfig()->get('show-left-time'))$this->broadcastMessage($this->getMiniGameApi()->getLanguage()->translateString('left.time',[intval($this->getRemainingRunTime()->asSec())]));
